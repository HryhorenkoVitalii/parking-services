...
class Parking:
    ...
    def car_come(
        self, car_number: str, permission: Permissions | None = None) -> int | ParkingException:
        if permission:
    ...
    def car_leave(self, car_number: str) -> int | ParkingException:
    ...
    def _get_available_priority(self, permission: Permissions) -> list[Permissions]:
        available_permission: list[Permissions] = []
    ...
    def add_new_place(
        self, new_place: dict[str, dict]) -> dict[str, dict] | ParkingException:

        regular = new_place.get("regular_nodes")
        permission = new_place.get("permission_nodes")
        if regular:
            for node in regular:
                self.regular_nodes.append(
                    ParkingNode(number_place=node["number_place"])
                )
        if permission:
            for node in permission:
                ...
                self.permission_nodes.append(
                    ParkingNode(
                        number_place=node["number_place"],
